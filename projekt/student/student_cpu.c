...
void phong_onInit(int32_t width,int32_t height){
  ...
  init_Vec3(&phong.lightPosition,100.f,100.f,100.f);
    ...
    VertexShader vertex_shader = phong_vertexShader;
    FragmentShader fragment_shader = phong_fragmentShader;
    ...
    cpu_attachVertexShader(phong.gpu, phong.program, vertex_shader);
    cpu_attachFragmentShader(phong.gpu, phong.program, fragment_shader);
    cpu_setAttributeInterpolation(phong.gpu, phong.program, 0, ATTRIB_VEC3, SMOOTH);
    cpu_createBuffers(phong.gpu, 1048, phong.buffer);
    cpu_bufferData(phong.gpu, 1048, sizeof(struct BunnyVertex)*1048, bunnyVertices);
    cpu_createVertexPullers(phong.gpu, 1, &phong.puller);

    for (size_t i = 0; i < 1048; i++) {
        cpu_setVertexPullerHead(phong.gpu, phong.puller, 0, phong.buffer[i], i*sizeof(float)*3, sizeof(float)*3);
...
void phong_onDraw(SDL_Surface*surface){
    ...
    cpu_drawTriangles(phong.gpu, 1048*3);
